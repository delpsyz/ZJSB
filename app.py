...
def call_deepseek(text, config):
    ...
    '---SEC3---',
    '项目简介:',
    'WD1-方案名称：复制原文中WD1方案的完整描述段落',
    'ZG1-方案名称：复制原文中ZG1方案的完整描述段落',
    '(继续列出每个方案,格式: 方案编号-方案名称：方案描述原文)',
    '实施成效:',
    '复制报告结论章节中关于实施成效的完整段落(至少3-5句话)',
    '',
    '---SEC4---',
    '以下每行代表一个方案,用|分隔21个字段,数值只填数字不含单位',
    'WD1 | 方案名称 | 方案内容描述 | 投资额 | 经济效益 | 节电量 | 折标煤 | CO2 | 原辅料 | 节水量 | COD | 氨氮 | 烟尘 | 粉尘 | SO2 | NOx | VOC | 固废 | 废液 | 重金属 | 其他',
    'ZG1 | 方案名称 | 方案内容描述 | 投资额 | 经济效益 | 节电量 | 折标煤 | CO2 | 原辅料 | 节水量 | COD | 氨氮 | 烟尘 | 粉尘 | SO2 | NOx | VOC | 固废 | 废液 | 重金属 | 其他',
    '(继续列出,每个方案一行,21个字段用|分隔,找不到的数据填0)',
    '',
    '---CITE---',
    '对每个有数值的字段,提供原文出处',
